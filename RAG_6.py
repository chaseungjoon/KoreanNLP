...
def ko_tokenize(text: str):
    return re.findall(r"[가-힣]+|[a-z0-9]+", text.lower())
def hybrid_query(question, retriever, bm25, k=5, alpha=0.7):
    ...
    return [retriever.passages[i] for i in top_idx]
"""
SBERT RETRIEVER
"""
...
def train(args):
    ...
    pass_tokens = [ko_tokenize(p) for p in passages]
    ...
    torch.save({"passages": passages, "embeddings": retriever.embeddings, "pass_tokens": pass_tokens}, args.output_dir + "/retriever.pt")

"""
INFERENCE
"""
def load_retriever(saved_dir: str, sbert_name="snunlp/KR-SBERT-V40K-klueNLI-augSTS") -> (SBERTRetriever, List[List[str]]):
...
def predict(args):
    ...
    bm25 = BM25Okapi(pass_tokens)
    reranker = CrossEncoder("bongsoo/klue-cross-encoder-v1", max_length=512)
    ...
    with open(args.output_path, "w", encoding="utf-8") as outf:
        for sample in tqdm(data, desc="Generating", unit="sample", total=len(data)):
            ...
            docs = hybrid_query(q, retriever, bm25, k=20) # Retrieve more docs
            rerank_scores = reranker.predict([(q, doc) for doc in docs])
            reranked_docs = [doc for _, doc in sorted(zip(rerank_scores, docs), reverse=True)]
            ctx = reranked_docs[:3] # Select top 3
            ctx_block = "\n".join(f"- {c}" for c in ctx)
            inst = INST.get(qtype, INST["서술형"])
            prompt = (
                f"{PROMPT}\n\n"
...
def main():
    ...
    p.add_argument("--output_dir", default="RAG6_ckpt")
    p.add_argument("--batch", type=int, default=1)
    p.add_argument("--epochs", type=int, default=3)
    p.add_argument("--grad_accum", type=int, default=8)

    p.add_argument("--test_path")
    p.add_argument("--adapter_path", default="RAG6_ckpt")
    p.add_argument("--output_path", default="submission_RAG6.jsonl")
